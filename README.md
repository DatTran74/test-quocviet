...
## Deploy to GitHub Pages and publish to Teams

Use this flow when you want to host the tab as a static web app on GitHub Pages.

1. Make sure the project has a GitHub remote and `gh-pages` branch permissions.

2. Build and deploy static files to GitHub Pages:

```bash
npm run deploy:ghpages
```

This command publishes files in `dist/ghpages` where the Teams tab page is generated at `tabs/home/index.html`.

3. Set Teams manifest values in [env/.env.githubpages](env/.env.githubpages):
- `TEAMS_APP_ID`: use the same app id you are publishing/updating.
- `TAB_ENDPOINT`: for project pages use `https://<github-user>.github.io/<repo-name>`.
- `TAB_DOMAIN`: for project pages use `<github-user>.github.io`.

4. Publish app package to your tenant using the GitHub Pages endpoint:

```bash
npx -y @microsoft/m365agentstoolkit-cli@latest publish --env-file ./env/.env.githubpages
```
